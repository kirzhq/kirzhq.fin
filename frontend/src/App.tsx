        ...
        <section className={`insight-layout ${month ? 'monthly-insights' : 'yearly-insights'}`}>
          ...
          <article className="card category-panel">
            ...
            <div className="category-panel-body">
              ...
              <div className="legend-list full-legend" style={{ '--legend-rows': Math.ceil((summary?.categoryPoints.length ?? 0) / 2) } as React.CSSProperties}>{(summary?.categoryPoints ?? []).map((point, index) => <div key={point.category} className={point.category === 'Еда' ? 'food-legend-row' : ''} role={point.category === 'Еда' ? 'button' : undefined} tabIndex={point.category === 'Еда' ? 0 : undefined} onClick={() => point.category === 'Еда' && setFoodDetailsOpen(true)} onKeyDown={(event) => { if (point.category === 'Еда' && (event.key === 'Enter' || event.key === ' ')) setFoodDetailsOpen(true); }}><i style={{ background: colors[index % colors.length] }} /><span>{point.category}</span><strong>{summary?.expense ? ((point.amount / summary.expense) * 100).toFixed(1) : '0'}%</strong><small>{formatMoney(point.amount)}</small></div>)}</div>
            </div>
          </article>
        </section>
