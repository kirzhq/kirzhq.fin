...
- статистика по Lada Vesta, отдельный учёт бензина и техобслуживания, экспорт расходов в Excel;
- журнал заправок с расчётом пробега, расхода в л/100 км и стоимости 100 км;
- импорт и экспорт полной резервной копии финансовых данных;
- светлая и тёмная темы;
- быстрое добавление операций командой iPhone через отдельный защищённый API;
- единая авторизация с домашним порталом `kirzhq.home`.
...
категории, операции, подкатегории автомобильных расходов, показания одометра и объём топлива,
автомобиль, долги и погашения. Passkey, пароли и секреты
...
Для приложения «Быстрые команды» предусмотрен отдельный токен с ограниченным
доступом. Он позволяет только получить список категорий и создать операцию:

```dotenv
SHORTCUT_API_TOKEN=случайный-секретный-токен
```

Запросы передают токен в заголовке `Authorization: Bearer ...`. Основная
авторизация сайта, пароль и passkey в команду не передаются. Токен можно в любой
момент заменить в `.env` и пересобрать backend.
